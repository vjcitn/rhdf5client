...
This document summarizes work to October 2018 in demonstrating
...
# Working with HDF Scalable Data Service (HSDS)

## HSDS interface with rhdf5client's REST operations

The primary software base for working with the HDF Scalable
Data Service is the h5pyd Python library.  Below we discuss
how to use that library with R.  Here we illustrate the
basic interfaces in rhdf5client.  John Readey of
the HDF Group has provided a public repository of HDF5
data in an HSDS folder called /shared/bioconductor
that
will be used here.

### Connecting to the server and discovering content

Use `HSDSSource` to create an object that
...
```{r lk1}
URL_hsds() # the 'endpoint' handling queries
src = HSDSSource( URL_hsds() )
listDomains(src, "/shared/bioconductor")
```

### Working with a dataset in a 'file'

The `HSDSFile` method provides focused access.
When a dataset is selected, square-brackets can
be used to obtain content.
```{r lk2}
f = HSDSFile(src, "/shared/bioconductor/darmgcls.h5")
f
...
### Working with data through the DelayedArray interface

The `HSDSArray` method gives a preview of
'corners' of the dataset.

```{r lkarr}
HSDSArray(URL_hsds(), "hsds", 
   "/shared/bioconductor/darmgcls.h5", "/assay001")
```


## HSDS operations via h5pyd
